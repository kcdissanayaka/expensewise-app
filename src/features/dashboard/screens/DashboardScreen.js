...
const DashboardScreen = ({ navigation }) => {
    ...
    monthlyStats: {
      currentMonth: (() => {
        try {
          ...
          console.warn('Error formatting initial month:', error);
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const isInitialized = await databaseService.ensureInitialized();
      if (!isInitialized) {
      ...
      const totalIncome = (incomeRecords || []).reduce((sum, income) => sum + (income?.amount || 0), 0);
      ...
      const recentExpenses = (expenseRecords || [])
        .filter(expense => expense && expense.created_at) // Filter out invalid records
      ...
      (expenseRecords || []).forEach(expense => {
        if (!expense || typeof expense.amount !== 'number') return; // Skip invalid records
      ...
      if (error.message.includes('Database not initialized')) {
        Alert.alert(
          'Database Error',
          'The app database is not properly initialized. Please restart the app.',
          [
            {
              text: 'Restart App',
              onPress: () => {
                // Navigate to splash or restart flow
                navigation.reset({
                  index: 0,
                  routes: [{ name: 'Login' }],
                });
        ...
        Alert.alert(
          'Connection Error',
          'There was a problem connecting to the database. Please try refreshing or restart the app.',
          [
            {
              text: 'Refresh',
